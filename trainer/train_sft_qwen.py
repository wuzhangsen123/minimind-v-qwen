"""
Stage 2: SFT Qwen-VLM with LoRA.
Train projector + LoRA adapters on LLM, freeze LLM base weights.
"""
...
def apply_lora(model, lora_rank=16, lora_alpha=32, lora_dropout=0.05):
    ...
    # Also add trainable params for gate/up/down projections
...
def train_epoch(epoch, loader, iters, start_step=0):
    ...
    for step, (input_ids, labels, pixel_values) in enumerate(loader, start=start_step + 1):
        ...
        if step % args.log_interval == 0 or step == actual_iters:
            ...
            log(f'SFT Epoch:[{epoch+1}/{args.epochs}]({step}/{actual_iters}) '
                f'loss:{cur_loss:.4f} lr:{cur_lr:.8f} eta:{eta:.0f}min')
        ...
        if (step % args.save_interval == 0 or step == actual_iters) and is_main_process():
            qwen_vlm_checkpoint(
                ...
                save_dir=args.checkpoint_dir, scaler=scaler,
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Qwen-VLM SFT with LoRA")
    ...
    parser.add_argument("--from_weight", default="pretrain_qwen", type=str)
    parser.add_argument("--save_weight", default="sft_qwen_lora", type=str)
    parser.add_argument("--from_resume", default=0, type=int, choices=[0, 1])
    parser.add_argument("--checkpoint_dir", default="../checkpoints", type=str)
    parser.add_argument("--use_wandb", action="store_true")
    parser.add_argument("--wandb_project", type=str, default="MiniMind-V-Qwen-SFT")
    ...
    model, tokenizer = init_qwen_vlm(
        from_weight=args.from_weight, device=args.device, freeze_llm=args.freeze_llm,
    )

    # Apply LoRA
    model = apply_lora(model, lora_rank=args.lora_r,
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb.init(project=args.wandb_project,
                   name=f'sft-qwen-bs{args.batch_size}-lr{args.learning_rate}-r{args.lora_r}')
        log('swanlab enabled')

    log(f'SFT+LoRA: {args.epochs} epochs, batch={args.batch_size}, '
        f'lr={args.learning_rate}, lora_r={args.lora_r}')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if skip > 0:
            ...
            train_epoch(epoch, loader, iters + skip, skip)
        else:
            train_epoch(epoch, loader, iters, 0)
